...
public static class GlobalSetting
{
    ...
    public static float globalNoteScale = 0.2f;
...
public class Main : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        //Music
        WWW a = new WWW("file://" + GlobalSetting.musicPath);
        //www.SendWebRequest();
        //var clip = DownloadHandlerAudioClip.GetContent(www);
        while (!a.isDone) { };
        music = a.GetAudioClip();

        a = new WWW("file://" + GlobalSetting.illustrationPath);
        //Illustration
        while (!a.isDone) { };
    ...
    void FixedUpdate()
    {
        Camera.main.orthographicSize = 5 / (Camera.main.aspect) * (aspect);
        GlobalSetting.noteSpeedFactor = 1 / (Camera.main.aspect) * (aspect);
        m_frames++;
        ...
        if (GlobalSetting.scoreCounter.combo >= 3)
            comboText.text = $"<size={(int)(Screen.height / 15)}>{GlobalSetting.scoreCounter.combo}</size>\n<size={(int)(Screen.height / 16 / 1.875f)}>{(GlobalSetting.autoPlay ? "AUTOPLAY" : "COMBO")}</size>";
        else
            comboText.text = "";
        scoreText.text = $"\n<size={(int)(Screen.height / 16)}>{Mathf.RoundToInt(GlobalSetting.scoreCounter.score).ToString().PadLeft(7, '0')}</size>  ";
    }
    ...
    private void init(string path)
    {
        ...
        foreach (judgeLine l in json.judgeLineList)
        {
            ...
            Debug.Log($"Line instatiated. ID: {i - 1}");
        }
    }
}
